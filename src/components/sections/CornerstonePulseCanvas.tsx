...
export interface CornerstonePulseCanvasHandle {
...
export const CornerstonePulseCanvas = forwardRef<
  CornerstonePulseCanvasHandle,
  Props
>(function CornerstonePulseCanvas({ left, top, width, height }, ref) {
  ...
  return (
    ...
    >
      {pulses.map((p) => (
        <span
          key={p.id}
          className={`cornerstone-pulse-strip cornerstone-pulse-strip--${p.edge}`}
          style={{ left: `${p.x - STRIP_WIDTH / 2}px` }}
        />
      ))}
      <style>{`
        .cornerstone-pulse-strip {
          position: absolute;
          ...
          animation: cornerstone-pulse-spread ${PULSE_LIFETIME_MS}ms cubic-bezier(0.2, 0.85, 0.3, 1) forwards;
          will-change: transform, opacity;
          pointer-events: none;
        }
        .cornerstone-pulse-strip--top    { top: -5px; }
        .cornerstone-pulse-strip--bottom { bottom: -5px; }
        @keyframes cornerstone-pulse-spread {
